                ...
                <div class="shadow-sm overflow-hidden my-8">
                    <table class="border-collapse table-auto w-full text-sm">
                        ...
                        <tr>
                            ...
                            <th class="border-b dark:border-slate-600 font-bold w-48 p-4 pl-8 pt-0 pb-3 text-base font-semibold text-slate-400 dark:text-slate-200 text-left">Chat Status</th>
                            <th class="border-b dark:border-slate-600 font-bold w-64 p-4 pr-8 pt-0 pb-3 text-base font-semibold text-slate-400 dark:text-slate-200 text-left">Team Page</th>
                            <th class="border-b dark:border-slate-600 font-bold w-24 p-4 pr-8 pt-0 pb-3 text-base font-semibold text-slate-400 dark:text-slate-200 text-left">Member</th>
                        </tr>
                        </thead>
                        <tbody class="bg-white dark:bg-slate-800">
                    ...
                    <c:forEach var="post" items="${mypostdata}">
                        <tr>
                            <td class="border-b border-slate-100 dark:border-slate-700 w-64 p-4 pl-8 text-base font-semibold text-slate-500 dark:text-slate-400">
                                ...
                                <c:choose>
                                    <c:when test="${post.chatroom_status == '1'}">
                                        <form class="chatForm" action="/chatstart/${post.proposal_id}" method="GET" onsubmit="openChatWindow(event)" style="margin-bottom: 0">
                                            <input type="submit" value="chatting" class="bg-blue-500 hover:bg-blue-600 text-white font-bold w-32 py-2 px-4 rounded">
                                            <input type="button" value="Delete Chat" data-proposal-id="${post.proposal_id}" class="chatRoomDeleteQuestion bg-red-400 hover:bg-red-500 text-white font-bold w-32 py-2 px-4 rounded">
                                        </form>
                                    </c:when>
                                    <c:otherwise>
                                        <input type="submit" value="Create Chat" data-proposal-id="${post.proposal_id}" class="chatRoomStartQuestion bg-blue-500 hover:bg-blue-600 text-white font-bold w-32 py-2 px-4 rounded">
                                    </c:otherwise>
                                </c:choose>
                            ...
                            <td class="border-b border-slate-100 dark:border-slate-700 w-64 p-4 pl-8 text-base font-semibold text-slate-500 dark:text-slate-400">
                                <c:choose>
                                    <c:when test="${post.teampage_status == '1'}">
                                        <input type="button" value="TeamPage" class="teamPageButton-${post.proposal_id} bg-indigo-400 hover:bg-indigo-500 text-white font-bold w-40 py-2 px-4 rounded">
                                        <input type="button" value="Delete TeamPage" data-proposal-id="${post.proposal_id}" class="DeleteTeamPageButton bg-indigo-400 hover:bg-indigo-500 text-white font-bold w-40 py-2 px-4 rounded">
                                    </c:when>
                                    <c:otherwise>
                                        <input type="button" value="create TeamPage" data-proposal-id="${post.proposal_id}" class="CreateTeamPageButton bg-indigo-400 hover:bg-indigo-500 text-white font-bold w-40 py-2 px-4 rounded">
                                    </c:otherwise>
                                </c:choose>
                            </td>
                            <td class="border-b border-slate-100 dark:border-slate-700 w-24 p-4 pl-8 text-base font-semibold text-slate-500 dark:text-slate-400">
                                <input type="button" value="View" data-proposal-id="${post.proposal_id}" class="showMembersBtn bg-gray-400 hover:bg-gray-500 text-white font-bold w-24 py-2 px-4 rounded">
                ...
                <c:forEach var="post" items="${myapplydata}">
                    <c:forEach var="status" items="${myapplystatus}">
                        <c:if test="${post.proposal_id eq status.proposal_id}">
                            ...
                            <c:if test="${status.status != '2' && post.chatroom_status == '1'}">
